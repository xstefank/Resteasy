...
{
   private Map<Class<? extends Annotation>, Set<Annotation>> stereotypes = Stereotypes.getStereotypes();
   
   @Override
   public <T extends Annotation> Class getClassWithAnnotation(Class<?> clazz, Class<T> annotation)
   {
      Class annotatatedClass = super.getClassWithAnnotation(clazz, annotation);

      if (annotatatedClass == null && findAnnotationInStereotypes(annotation, null, clazz) != null)
      {
         return clazz;
      }

      return annotatatedClass;
   }
   ...
   public <T extends Annotation> T getAnnotationFromResourceMethod(Class<T> annotationClass, Method method, ResourceClass resourceClass)
   {
      ...
      if (annotation == null && !stereotypes.isEmpty())
      {
   ...
   {
      Set<Class<T>> resourceStereotypes = collectStereotypes(method, clazz);
   ...
   private <T extends Annotation> Set<Class<T>> collectStereotypes(Method method, Class<?> clazz)
   {
