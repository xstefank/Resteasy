...
import org.jboss.resteasy.test.cdi.stereotype.resource.MediaTypeStereotype;
 ...
 * @tpTestCaseDetails Test for {@link javax.ws.rs.Produces} and {@link javax.ws.rs.Consumes} annotations
 * placed in the {@link javax.enterprise.inject.Stereotype} beans
...
public class MediaTypeStereotypeTest
{
   ...
   public void testProducesInStereotype()
   {
      ...
      Assert.assertEquals("Wrong content of the response", "{}",
   ...
   {
      WebTarget base = client.target(generateURL("/mediatype"));
      Response response = base.path("consumes")
            .request().header("Content-type", MediaType.APPLICATION_XML).post(Entity.xml("<>"));
